    ...
    <>
      ...
      <Grid container>
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
              {
               ...
               (
                 addresses.all.map((addressData) => (
                   <Typography my="1rem"
                    className={(addresses.selected === addressData._id)? "address-item selected" : "address-item not-selected"}
                    onClick={() => setAddresses({ ...addresses, selected: addressData._id})}
                   >
                      {addressData.address}
                      <Button startIcon={<Delete />} onClick={() => deleteAddress(token, addressData._id)}>DELETE</Button>
                   </Typography>
                 ))
               )
               :
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          ...
          <Box className="cart">
            <Box color="#3C3C3C" fontWeight="700" fontSize="1.2rem" padding={"1rem"} alignSelf="center">
              Order Details
            </Box>
            <Box paddingX="1rem" paddingY="0.5rem" display="flex" justifyContent="space-between" alignItems="center">
            ...
            <Box padding="1rem" display="flex" justifyContent="space-between" alignItems="center">
              <Box color="#3C3C3C" fontWeight="700" fontSize="1.2rem" alignSelf="center">Total</Box>
              <Box color="#3C3C3C" fontWeight="700" fontSize="1.2rem" alignSelf="center">${getTotalCartValue(items)}</Box>
            </Box>
